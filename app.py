...
# Tratamento do link para conversão automática em formato CSV de leitura
def obter_url_leitura(url):
    if "docs.google.com/spreadsheets/d/" in url:
        id_planilha = url.split("/d/")[1].split("/")[0]
        return f"https://docs.google.com/spreadsheets/d/{id_planilha}/export?format=csv"
    return ""

# Função de leitura robusta com tratamento de dados vazios
def carregar_dados():
    url_csv = obter_url_leitura(URL_PLANILHA)
    if url_csv:
        try:
...
# Gerencia o estado dos dados na sessão atual
if 'df_chamados' not in st.session_state:
    st.session_state.df_chamados = carregar_dados()

# Função para atualizar o estado visual da aplicação
def atualizar_dados_locais(df_novo):
    ...
    with st.form("form_chamado", clear_on_submit=True):
        ...
        if botao_salvar:
            if id_ticket and solicitante and descricao and atendente:
                if not st.session_state.df_chamados.empty and id_ticket in st.session_state.df_chamados["ID Ticket"].values:
                    ...
                    atualizar_dados_locais(df_atualizado)
